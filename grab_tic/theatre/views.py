...
from shows.models import Movies

from . models import OngoingShows,ShowDate,ScreenDateTime,Seat,Bookings
...
class TheatreListView(View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        all_dates = ShowDate.objects.filter(screendatetime__ongoingshows__in = ongoing_shows,date__gte = today).distinct().order_by('date')
...
class ScreenSeatsView(View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        movie = Movies.objects.get(uuid=movie_uuid)

        screen_date_time = ScreenDateTime.objects.get(uuid=screen_date_time_uuid)
